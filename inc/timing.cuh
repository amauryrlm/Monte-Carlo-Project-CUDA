...
void nmc_gpu_one_kernel_blocks_vs_time() {

    INIT_SIM(1000, 100, 1024, "nmc_gpu_one_kernel_blocks.csv", "n_blocks", 5);

    option_data.N_PATHS_INNER = 1000;
    cudaMemcpyToSymbol(d_OptionData, &option_data, sizeof(OptionData));

    std::function<void(float)> nmc_callback = [&] (float n_blocks) {
        auto price = wrapper_gpu_bullet_option_nmc_one_kernel(option_data, n_threads_per_block, n_blocks);
    };

    auto block_range = linspace(10, 10000, 100);
...
void nmc_gpu_one_point_blocks_vs_time() {

    INIT_SIM(1000, 100, 1024, "nmc_gpu_one_point_blocks.csv", "n_blocks", 5);
...
void nmc_gpu_optimal_blocks_vs_time() {
    ...
    std::string output_csv = "nmc_gpu_optimal_blocks.csv";
    std::string input_name = "n_blocks";
    int n_trials = 5;
